...
%% 
%% --------------------------------------------------------------------
-record(state,{myip,myport,masters}).
...
start(IpAddr,MasterList)-> gen_server:start_link({local, ?MODULE}, ?MODULE, [IpAddr,MasterList], []).
...
%
%% --------------------------------------------------------------------
init([{IpAddr,Port},MasterList]) ->
    spawn(fun()->h_beat(?HB_INTERVAL) end),
    io:format("~p~n~n",[{{IpAddr,Port},MasterList,?MODULE,?LINE}]),
    {ok, #state{myip=IpAddr,myport=Port,masters=MasterList}}.   
